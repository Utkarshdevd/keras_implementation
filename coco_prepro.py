...
{'captions': [u'A man with a red helmet on a small moped on a dirt road. ',
  ...
  u'A dirt path with a young person on a motor bike rests to the foreground of a verdant area with a bridge and a background
  of cloud-wreathed mountains. ', u'A man in a red shirt and a red hat is on a motorcycle on a hill side.'],
...
def prepro_captions(imgs,start='<start>',end='<eos>'):
  ...
  for i,img in enumerate(imgs):
    ...
    for j,s in enumerate(img['captions']):
      ...
      stnc.extend(txt)
      stnc.extend([end])
      img['processed_tokens'].append(stnc)
      if i < 10 and j == 0: print (stnc)
...
def encode_captions(imgs, params, wtoi):
  ...
  print ('encoded captions to array of size {}'.format(L.shape))
  return L, label_start_ix, label_end_ix, label_length

def main(params):

  imgs = json.load(open(os.path.expanduser(params['input_json']), 'r'))
...
if __name__ == "__main__":
  ...
  # input json
  parser.add_argument('--input_json', default='~/datasets/coco/coco_raw.json', help='input json file to process into hdf5')
  ...
  parser.add_argument('--images_root', default='~/datasets/coco/images/', help='root location in which images are stored, to be prepended to file_path in input json')
  parser.add_argument('--word_count_threshold', default=5, type=int, help='only words that occur more than this number of times will be put in vocab')
  parser.add_argument('--num_test', default=2000, type=int, help='number of test images (to withold until very very end)')
